...
(function($){
	...
	var defalutOpt={
		...
		defalutNum:0,//滑动点默认值
		decimal:0,//保留几位小数
		callBack:function(value){//滑动时的执行函数
			//console.log(value);
		}
	}
  ...
  return this.each(function(e) {
		...
		if(!isMobile()){//for pc
			...
			handler.mousedown(function(evt){
				...
				$(document).on("mousemove",function(e){
					...
					thisText.val(value);
					cross.css({width:handler.css("left")});
					opt.callBack(value);
				});
			});
			...
			handler[0].addEventListener('touchmove',function(event) {
				...
				{handler.css({left:(event.targetTouches[0].pageX-thisX)+"px"});}
				var value=(parseFloat(handler.position().left/sliderW)*(opt.max-opt.min)+opt.min).toString();
				if (value.indexOf(".")>0||opt.decimal>0){
				 	value = value.substr(0,value.indexOf(".")+opt.decimal);
				 	if(value<0){value=0}
		...
		//初始化函数
		function init(){
			thisText.val(opt.defalutNum);
			handler.css({backgroundColor:opt.handlerC,left:([(opt.defalutNum-opt.min)/(opt.max-opt.min)]*sliderW)+"px"});
			cross.css({backgroundColor:opt.crossC,width:handler.css("left")});
		}
